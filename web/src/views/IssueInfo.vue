...
const getBookIssue = async (id) => {
  ...
  published_at = moment.utc(bookInfo.value.published_at).local().format("YYYY-MM-DD HH:mm:ss")
  // 获取二级市场挂单列表
  getTradesList()
...
const goShare = () => {
  ...
  window.open(`https://twitter.com/intent/tweet?text=I'm looking at the rare edition ${title} book released by ${name}, so come and check it out on #dbookmarket. #BookLovers #author
   ${href}`)
}
...
const buyNow = async () => {
  ...
  const price = bookInfo.value.trade.price
  ...
  if (currentBuyLevel === 1) {
    ...
    if (provider) {
      ...
      const is_approve = await walletInstance.allowance(signer, block_chain, currency.toLowerCase())
      console.log("is_approve", is_approve == 0)
      console.log("is_approve", is_approve)
      // if (is_approve == 0) {
        const transactions = await walletInstance.approve(signer, block_chain, currency.toLowerCase())
  ...
  if (currentBuyLevel === 2) {
    ...
    if (provider) {
      ...
      let free = await walletInstance.getFee(signer, bookInfo.value.token.block_chain)
      console.log("free", free)
      const is_approve = await walletInstance.allowance(signer, bookInfo.value.token.block_chain, currency.toLowerCase())
      console.log("is_approve", is_approve == 0)
      console.log("is_approve", is_approve)
      if (is_approve == 0) {
        const transactions = await walletInstance.approve(signer, bookInfo.value.token.block_chain, currency.toLowerCase())
        console.log(transactions)
      }
      const transaction = await walletInstance.trade(signer, bookInfo.value.token.block_chain, currentBuyInfo.user.address, currentBuyInfo.issue.book.id, amount, "0x1234", price)
  ...
  <div class="main">
    ...
    <div class="content">
      <div class="issueInfo">
        ...
        <div class="infos">
          ...
          <div class="line2">
            <span class="author" @click="goAuthor">{{ bookInfo.book.author.name }}</span>
            <span class="address">{{ encate(bookInfo.book.author.address) }}</span>
          </div>
          <div class="line3" :title="bookInfo.book.desc || '什么也没有留下'">{{ bookInfo.book.desc || '什么也没有留下' }}</div>
          <div class="line4">
      ...
      <div class="Publication bookCard">
        <div class="topWrap">
          ...
          <!-- 售卖中 -->
          <div class="timeClock" v-show="['pre_sale', 'on_sale'].includes(bookInfo.status)">
            <div v-show="['on_sale'].includes(bookInfo.status)" class="clockTip">Supply countdown</div>
            <div class="cl_1 part">
      ...
      <div class="Destroyed bookCard">
        ...
        <div class="cardBottom">
          <div class="left_1">
            ...
            <div class="value">{{ bookInfo.quantity - bookInfo.n_circulations }}</div>
          </div>
          <div class="left_2">
          ...
          <div class="left_3">
            ...
            <div class="value_">{{ bookInfo.destroy_log == '' ? '待销毁' : '已销毁' }}</div>
          </div>
        </div>
      </div>
      <div class="Secondary ">
        ...
        <div class="card_Secondary">
          <div v-show="bookInfo.status == 'on_sale'" class="shoufa">
            <img src="../assets/img/state_2.svg" alt="">
            <div>首发供应结束后开启</div>
          </div>
          <div v-show="bookInfo.status !== 'on_sale'" class="erji">
            ...
            <div v-if="naviType === 'List'" class="ListWrap">
              <div v-for="item in tradesList" :key="item.id" class="listItem">
                <el-image :src="item.user.avatar_url"></el-image>
                <div class="addressWrap">
                  <div>{{ encate(item.user.address) }}</div>
                  <div>{{ item.updated_at }}</div>
                </div>
                <div class="buyWrap">
                  <div class="price">{{ item.price }}{{ bookInfo.token?.currency }}</div>
                ...
                <el-table-column label="Unit Price" prop="price">
                  <template #default="scope">
                    <div class="evenetLable">
                      ...
                      <span>{{ scope.row.price }}{{ bookInfo.token?.currency }}</span>
                    </div>
                  </template>
                </el-table-column>
    ...
    <div class="issueTips" v-show="issueType === 'new'">
      <div class="inner">
        ...
        <div class="i_content">Congratulations on your successful listing, the last step, hurry up and share it on
          Twitter to let your readers and fans know.</div>
        <div class="operaIcon">
      ...
      <div class="inner">
        ...
        <div class="formWrap">
          <div class="form_item">
            ...
            <div class="form_value">{{ bookInfo.price }}{{ bookInfo.token?.currency }}</div>
          </div>
          <div class="form_item">
