...
class Wallet {
  constructor(blockChain) {
    ...
    this.provider = new WalletConnectProvider({
      ...
      rpc: {
        ...
        3141: 'https://api.hyperspace.node.glif.io/rpc/v1'
      },
    ...
    this.ethers_provider = new ethers.providers.Web3Provider(this.provider);
    this.signer = this.ethers_provider.getSigner();
    this.web3Instance = new web3(this.provider) // 取小狐狸的地址
    this.address = this.provider.accounts[0]
  ...
  async isApprovedForAll(blockChain) {
    try {
      console.log(blockChain)
      const nftContract = new this.web3Instance.eth.Contract(contractAbi.nft, contractAddress[blockChain].nft);
      let res = await nftContract.methods.isApprovedForAll(this.address, contractAddress[blockChain].platform).call();
  ...
  async approveIssue(blockChain, approved = true) {
    try {
      const nftContract = new this.web3Instance.eth.Contract(contractAbi.nft, contractAddress[blockChain].nft);
      // let ethContract = this.web3Instance.eth.Contract(contractAbi.nft,contractAddress.platform) //所有代币的abi可以通用（abi,合约地址）
      let res = await nftContract.methods.setApprovalForAll(contractAddress[blockChain].platform, approved).send({
        from: this.address
      });
      return res;
    } catch (e) {
  ...
  async issue(blockChain, nftId, amount, metadata, price, ratio) {
    /**
     ...
     */
    try {
      const platformContract = new this.web3Instance.eth.Contract(contractAbi.platform, contractAddress[blockChain].platform);
      const seller = this.address;
      let _price = this.toWei(parseFloat(price));
      let res = await platformContract.methods.issue(seller, nftId, amount, metadata, _price, seller, parseFloat(ratio) * 100).send({
        from: this.address
      });
      return res;
    } catch (e) {
  ...
  async allowance(blockChain, symbol) {
    try {
      const usdcContract = new this.web3Instance.eth.Contract(contractAbi[symbol], contractAddress[blockChain][symbol]);
      const address = this.address;
      console.log(address)
      console.log(contractAddress[blockChain].platform)
      let res = await usdcContract.methods.allowance(address, contractAddress[blockChain].platform).call();
      return res;
    } catch (e) {
  ...
  async approve(blockChain, symbol) {
    /**
    ...
    try {
      ...
      const usdcContract = new this.web3Instance.eth.Contract(contractAbi[symbol], contractAddress[blockChain][symbol]);
      let res = await usdcContract.methods.approve(contractAddress[blockChain].platform, MAX_INT).send({
        from: this.address
      });
      return res;
    } catch (e) {
  ...
  async trade(blockChain, seller, nftId, amount, metadata, price, fee = 0) {
    /**
    ...
    try {
      ...
      let _fee = await this.getFee(blockChain); // wei
      ...
      console.log("tradeValue-mo", tradeValue)
      const platformContract = new this.web3Instance.eth.Contract(contractAbi.platform, contractAddress[blockChain].platform);
      const buyer = this.address;

      let res = await platformContract.methods.trade(seller, buyer, nftId, amount, metadata, tradeValue, fee).send({
        from: this.address,
        value: tradeValue,
        gasPrice: this.web3Instance.utils.toHex(this.web3Instance.utils.toWei('10', 'gwei')),
        gasLimit: this.web3Instance.utils.toHex(3000000),
      });
      return res;
    } catch (e) {
  ...
  async tradev2(signer, blockChain, seller, nftId, amount, metadata, price, fee = 0) {
    /**
     * For the second-class market.
     * if res.status == 1, that means the transaction is successful, otherwise, failed.
     * 
    ...
    try {
      // get platform fee from contract
      let _fee = await this.getFeev2(signer, blockChain); // wei
  ...
  async getFee(blockChain) {
    try {
      const platformContract = new this.web3Instance.eth.Contract(contractAbi.platform, contractAddress[blockChain].platform);
      let fee = await platformContract.methods.getFee().call();
      console.log("fee", fee)
      return fee;
    } catch (e) {
  ...
  async getFeev2(signer, blockChain) {
    try {
      const platformContract = new ethers.Contract(contractAddress[blockChain].platform, contractAbi.platform, signer);
  ...
  async payFirstTrade(amount, price) {
    ...
    try {
      ...
      const platformContract = new this.web3Instance.eth.Contract(contractAbi.platform, contractAddress[blockChain].platform);
      let res = await platformContract.methods.payFirstTrade(tradeValue).send({
        from: this.address,
        value: tradeValue
      });;
      return res;
